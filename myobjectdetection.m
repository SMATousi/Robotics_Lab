function [area, centroids, orientation] = myobjectdetection(image)
    
left_image = imread(image);
...
area = cat(1, centroid_left.Area);
centroids = cat(1, centroid_left.Centroid);
orientation = cat(1, centroid_left.Orientation);
end